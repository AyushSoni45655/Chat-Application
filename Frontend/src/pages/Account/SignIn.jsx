...
const SignIn = () => {
  ...
  const {setSignIn,loading,signIn,showPassword,setShowPassword,signInOnChange,signInOnSubmit} = useContext(Contextt);
  const submitter = async(e)=>{
    e.preventDefault();
    const success = await signInOnSubmit();
    if(success)navigate("/")
  }
  // ✅ Password validation conditions
  const validations = {
    length: signIn.password.length >= 8,
    upper: /[A-Z]/.test(signIn.password),
    lower: /[a-z]/.test(signIn.password),
    digit: /[0-9]/.test(signIn.password),
    special: /[!@#$%^&*(),.?":{}|<>]/.test(signIn.password),
  };
        ...
        >
          ...
          <div className="flex justify-end">
            <NavLink to="/forgotPassword">
              <p className="text-sm sm:text-base font-semibold text-[#fd4a31] hover:underline">
                Forgot Password?
              </p>
            </NavLink>
          </div>
